...
//// This test reflects the ability of the model to simulate the Natural Modes of string-object.
//// - Test is successful.
//*******************************************************************/
...
//float M = 60; // total mass of sheet
//int N = 80; // number of control points used to represent the sheet
//float stiff = 500; // stiffness of each spring in the sheet (overall stiffness = value given/N)
...
//    sheet.prtcl[N-1].makeFixed(); // fix the ending point
...
//    for (int i = 1; i < N; i++) {
//      y[i] = (L/(N-1)) + y[i-1];
//      x[i] = sinAmp * sin(sinN*PI*(y[i]-offsetY)/L) + offsetX;
//      sheet.prtcl[i].updatePosition(x[i], y[i]);
//    }  
